I want to build an app for myself that allows me to have a complete project management platform like Jira, Confluence, and all other apps, but built specifically for my personal use in lifeOS and any member of lifeOS. It will have first-class citizen support for AI agents, which would mean that not only lifeOS but any other project that I work on, including finance ops, FtryOS, or any future projects, I would want to have an application that allows me to track the entire project, its milestones, epics, stories, tasks, and bugs, all of them like an industry-standard project management platform.

The name of the application will be Control Pane written as CtrlPane and we have the domain as ctrlpane.com

The goal is to make this multi-tenant right from day one and multi-user as well because I might give access to this to my company employees as well or we might entirely switch to this application and leave Jira behind in the future. Hence we need to architect this to scale to that level
...
Control pane should also allow easy integration with the following applications like Slack, Jira, Telegram, Google Workspace, so that I can connect all my accounts of these platforms. From where we can always pull data for intelligent knowledge base and also push back to it so that we can keep them in sync. As these are my private accounts, I'll have full access to integrate these applications with Control Pane
